...
/// 옵셔널 체이닝의 결과 타입
/// - 속성(프로퍼티) 접근 시 -> 원래 타입이 옵셔널로 감싸져서 변환됨
/// - 메서드 호출시 -> 리턴값이 있으면 옵셔널로, 없으면 그냥 void처럼 작동
let name: String? = person.dog?.name

/// 중첩된 옵셔널 체이닝
/// 여러 단계 연속으로 체니잉이 가능하다
class Company{
  var ceo: Person?
}
...
let dogName = compnay.ceo?.dog?.name
print(dogName)

/// 정리 : 옵셔널 체이닝은
/// 1. nil 안전하게 처리
/// 2. 코드 간 결과
/// 3. if let, gaurd let으로 매번 풀지(바인딩하지) 않아도 된다.

// 옵셔널 체이닝은 마지막에 올 수 없다.
// dog?
